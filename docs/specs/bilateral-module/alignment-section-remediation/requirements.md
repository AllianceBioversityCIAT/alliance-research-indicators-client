...
- **Current state** (per §6 audit): `PatchAlignmentResult` already carries `fieldErrors?: Record<string, string>`; the component has an `inlineErrors` signal; the toast for `/pool-funding-alignment` 400s is already suppressed. **But the parser that populates `fieldErrors` cannot capture this field as-is**: `extractFieldErrors` (`bilateral.service.ts:117-131`) requires `errorDetail.errors` to be a **stringified-JSON string** and keeps **only string-valued** entries (`typeof v === 'string'`). `unknown_sp_codes` is a **`string[]`** — it is silently dropped. Compounding it, `ErrorResponse.errors` is typed `string` (`responses.interface.ts:17`), so if the backend returns `errors` as an object the guard short-circuits. So this is **not** a no-op key-wire: the parser (and possibly the `ErrorResponse` type) must be extended. Confirm the live envelope against `AC-1594-bilateral-module-v2` before choosing the approach.
- **Acceptance criteria**:
  - AC-03.1 — PATCH alignment `400` with `errorDetail.errors.unknown_sp_codes: ["SPxx", ...]` → the rejected codes are extracted (extend `extractFieldErrors` to handle the array, or carry a dedicated `unknownSpCodes: string[]` on `PatchAlignmentResult`) and surfaced as the picker's inline error — not a generic toast (the URL-scoped 400 exception already suppresses the toast for `/pool-funding-alignment`).
