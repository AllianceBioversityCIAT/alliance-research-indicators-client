...
- **2026-06-10 — D-6a Destructive-confirm uses the actual house pattern `ActionsService.showGlobalAlert({ confirmCallback, cancelCallback })`, not PrimeNG `p-confirmdialog`.** Clarifies D-6. Discovered during T-BIL-TM2-04: the repo has **no** `ConfirmationService`/`p-confirmdialog` usage anywhere; the real, established destructive-action pattern is `ActionsService.showGlobalAlert` (rendered by the `global-alert` component, `severity: 'delete'`, with `confirmCallback`/`cancelCallback`) — used 3× already in `actions.service.ts`. D-6's intent ("house destructive-action pattern") is honored; only the named widget was wrong. Resolution: the page calls `showGlobalAlert` on a destructive SP-deselect — confirm applies the removal (drops the SP + its draft), cancel re-keeps the SP. Copy per OQ-5 default.
