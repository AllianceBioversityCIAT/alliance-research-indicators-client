...
### T-BIL-ITG-06 — Regression/a11y/perf/bundle sweep — [~] automatable portion ✅ PASS (attempt 1) — 2026-07-02 · manual golden path PENDING

- **Attempts**: 1 Implementer run, 1 Reviewer run (automatable portion; manual live golden path deferred to the user by Leader decision).
- **Files changed** (3, test-only, additive): page spec, block spec, util spec.
- **What was added**: AC-06.1 strict payload test (guidance fixture + cross-type draft; full-array equality + exact 6-key DTO set + `JSON.stringify` leak sweep for `badge|classification|hasTypeMatch`); AC-06.3 passive-lock test (warning renders with zero interactive descendants; all selects disabled `aria-disabled`/`tabindex="-1"`; guidance computeds live); NF-01 perf test (warm-up + 100-iteration average < 5 ms over the largest catalog); NF-02 gap assertions (text-only group headers; buttons `type="button"`, never `tabindex="-1"`); `afterAll` matchMedia restores in both stubbed describes.
- **Implementer verification**: full `npm run test` → 5926 passed / 1 failed (solo el pre-existente `multiselect.component.spec.ts`, stash-control re-verificado); `npm run test:coverage` → **99.14 / 97.78 / 98.62 / 99.44** (floors 40/20/45/30); lint clean; `npm run build` exit 0 con exactamente los 9 warnings pre-existentes (md5-diff vs baseline).
- **NF-03 bundle evidence**: lazy chunk `pool-funding-alignment-component` 53.30 kB raw / **12.18 kB gzip** vs baseline `5b1f5dd0` 45.90 / 10.75 ⇒ **delta +1.43 kB gzip ≤ 5 KB** ✅ (baseline construido desde `git archive` @ `5b1f5dd0` con el mismo `node_modules`; sound — package-lock/angular.json sin cambios entre baseline y HEAD). Initial bundle sin cambio (264.86 vs 264.88 kB, ruido).
