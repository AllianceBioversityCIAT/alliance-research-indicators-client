...
### Entry 11 — SP picker switch (Levers → Science Programs) + AI-card mount

| Field | Value |
| --- | --- |
| Status | ✅ completed |
| Date | 2026-05-26 |
| Method | Direct follow-up commits after the indicator-mapping spec rewrite identified the SP → AOW → HLO tree as the canonical target shape |
| Commits | <ul><li>`691fc99d` — `✨ feat(bilateral-module/alignment-section): switch SP picker from Levers to Science Programs (Figma 32471:129337)`</li><li>`fc56e0b1` — `🎨 feat(bilateral-module/alignment-section): SP picker visual alignment + HLO action card wiring (Figma 32471:129636)`</li></ul> |
| Triggered by | The sibling [`../indicator-mapping/`](../indicator-mapping/) spec's mockup-first rewrite (2026-05-23) which made the **SP → AOW → HLO tree** the canonical hierarchy. The alignment-section picker had been built against the broader CLARISA `levers` source for parity with `alliance-alignment` / `my-projects`, but that scope is wider than the bilateral flow needs and would have produced an `(indicator_code, lever_code)` pairing surface that doesn't map cleanly to the modal's sidebar. Narrowing the source to Science Programs aligns the saved data with the downstream HLO modal's sidebar. |
| Changes | <ul><li>**SP picker source**: `<app-multiselect>` `serviceName` switched from `levers` to the Science-Programs source. AC-04.1 in `./requirements.md` originally cited `levers`; the narrower SP source is still CLARISA-owned (PRD C-3 holds — no new vocabulary). The `lever_codes` field on `UpdatePoolFundingAlignmentDto` is preserved for back-compat with the backend contract; semantically these codes are now SP codes.</li><li>**Visual alignment with Figma `32471:129337`**: SP picker label, placeholder, and required-marker pattern reconciled with the mockup; resolves AD-2 holdover from Entry 10 in a cleaner second pass.</li><li>**AI-card mount (host work for sibling spec)**: `fc56e0b1` also wired the `BilateralActionCardComponent` (sibling-spec T-BIL-IM-03) into the alignment-section template — CTA click calls `hloSelectionModalContextService.setContext({ resultCode }) + allModalsService.openModal('hloSelection')`. The card itself + the modal context service ship under the sibling spec; this commit only places the host instance in the alignment section because the alignment section IS the visual host. The remaining T-BIL-IM-10 scope (HLO-card iteration loop + catalog-empty fallback) stays gated under the sibling spec's OQ-IM-1/2/3.</li></ul> |
| Files modified | `pool-funding-alignment.component.ts`, `pool-funding-alignment.component.html`, `pool-funding-alignment.component.spec.ts` |
| Mockups consulted | [`../figma-mockups/32471-129337-pool-funding-alignment-sp-dropdown-open.md`](../figma-mockups/32471-129337-pool-funding-alignment-sp-dropdown-open.md), [`../figma-mockups/32471-129636-pool-funding-alignment-sp-selected-hlo-prompt.md`](../figma-mockups/32471-129636-pool-funding-alignment-sp-selected-hlo-prompt.md) |
| Decisions | Captured in [`./design.md` §11](./design.md#11-design-decisions-decision-record) 2026-05-26 row. |
| Verification | Component spec passes; `ng build` clean (per the commit-time CI). Cross-spec validation: [`../indicator-mapping/validation-report.md` §3 T-BIL-IM-10](../indicator-mapping/validation-report.md) notes the partial-discharge from this commit. |
| Drift acknowledgement | `./requirements.md` AC-04.1 text still references "Lever picker" / "`levers` source". This is intentionally not retro-edited — the requirements doc is the pre-mockup-remediation snapshot; the canonical post-shipping shape lives in this execution log + the §11 design decision row. The implementation matches the mockups; the documentation captures the journey. |

---

## 4. Summary

**Status**: ✅ all 14 alignment-section tasks shipped and verified in code as of 2026-05-24. Initial bundle (`17417fdd`) + 6 remediation commits (`01a0cd57`, `352299ab`, `86252209`, `3df3deff`, `05fa2913`, `e07ec9fb`, `9b946f9f`) all live on `origin/AC-1594-bilateral-module`. Post-shipping evolution: SP picker switched from Levers → Science Programs in `691fc99d` + `fc56e0b1` (2026-05-26, Entry 11).
