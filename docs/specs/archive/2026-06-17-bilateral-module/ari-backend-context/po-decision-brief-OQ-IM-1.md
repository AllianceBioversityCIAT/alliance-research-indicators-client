...
> | Source path | `docs/specs/bilateral-module/indicator-mapping/po-decision-brief-OQ-IM-1.md` |
> | Source branch | `AC-1594-bilateral-module-v2` |
> | Source commit | *(uncommitted at snapshot time — backend agent drafted on 2026-05-27; commit hash to be filled when the backend lands it)* |
> | Source date | 2026-05-27 |
> | Snapshot taken | 2026-05-27 (by STAR / `AC-1594-bilateral-module`) |
> | Purpose | One-pager for PO to decide OQ-IM-1 (retire R-BIL-031 or affirm it). Path-agnostic — does not assume a direction. |
> | Responds to | [`../indicator-mapping/open-questions-for-ba.md` §7.1](../indicator-mapping/open-questions-for-ba.md#71-backends-verdict-per-oq) (where the PO escalation was first flagged) + the FE's request on 2026-05-27 for a digestible brief to send upstream |
>
> **How to read this file.**
> The body below is a verbatim copy of the backend-drafted brief. Relative links inside (`./backend-response-to-fe.md`, `../requirements.md`, `../design.md`, `../frontend-handoff.md`, `../frontend-data-model.md`) resolve **inside the ARI backend repo**, not from this location. To follow them, open the source at `alliance-research-indicators-main/docs/specs/bilateral-module/indicator-mapping/po-decision-brief-OQ-IM-1.md`.
>
> **Two precision corrections the brief makes** (vs the original FE framing in `open-questions-for-ba.md` §7.1):
> 1. **R-BIL-031 only** — D5 + D12 become downstream consequences under Path A, not separate decisions the PO has to retire. See §8 of the brief.
> 2. **Zero search-team impact** — backend verified the entity's `@OpenSearchProperty` decorators only index `lever_code`, `indicator_code`, `indicator_type`, `is_stale`. The polymorphic FKs aren't individually indexed. Removes one of the perceived blockers. See §7 of the brief.
>
> Both corrections are propagated into our FE specs in the same commit that snapshotted this file.
